...
list all file with `setuid`

```bash
find / -user root -perm -4000 -exec ls -ldb {} \; 2>/dev/null | tee -a setuid.txt
```

the result will be stored on this file….

then compare result with usually files found on the system
...
-rwsr-xr-x 1 root root 94240 Jun  9 14:54 /sbin/mount.nfs" > known.txt
```

> compering command

```bash
grep -vxFf known.txt setuid.txt
```
